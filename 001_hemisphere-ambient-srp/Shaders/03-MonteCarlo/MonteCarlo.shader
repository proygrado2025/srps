Shader "Custom SRP/Monte Carlo Material"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        // SetShaderPass must use this name in order to execute the ray tracing shaders from this Pass.
            ...
            //calculate fresnel
            float Calculatefresnel(const float3 I, const float3 N, float etai, float etat)
            {
                float kr;
            ...
            void ClosestHitMain(inout RayPayload payload : SV_RayPayload, in AttributeData attribs : SV_IntersectionAttributes)
            {
                ...
                if (blender < 1 - _SpecTrans){ // BRDF
                    ...
                    float etai = fromOutside ? 1 : 1.55;
                    float etat = fromOutside ? 1.55 : 1;
                    ...
                    float kr = Calculatefresnel(WorldRayDirection(), faceNormal, etai, etat);
